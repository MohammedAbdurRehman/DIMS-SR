    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card shadow-md border-b border-border sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 sm:py-6">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-primary hover:underline font-semibold mb-4"
          >
        ...
        <div className="bg-card rounded-2xl p-6 sm:p-8 shadow-lg border border-border">
          {activeTab === 'email' && (
            <form onSubmit={handleEmailChange} className="space-y-6">
              ...
              <button disabled={loading} className="w-full bg-primary text-white py-3 rounded-lg font-bold disabled:opacity-50 transition-colors">
          ...
          {activeTab === 'mfa' && (
            <div className="space-y-6">
              ...
              <button
                onClick={() => onMfaChange?.()}
                className="w-full bg-primary text-white py-3 rounded-lg font-bold transition-colors"
              >
          ...
          {/* Feedback Messages */}
          {error && (
            <div className="mt-6 p-4 bg-destructive/10 border border-destructive/20 text-destructive rounded-lg flex items-center gap-2 animate-in fade-in slide-in-from-top-1">
              <X size={18} /> <span className="text-sm font-semibold">{error}</span>
            </div>
          )}
          {success && (
            <div className="mt-6 p-4 bg-green-500/10 border border-green-500/20 text-green-600 rounded-lg flex items-center gap-2">
              <Check size={18} /> <span className="text-sm font-semibold">{success}</span>
            </div>
          )}
        </div>
      </main>
